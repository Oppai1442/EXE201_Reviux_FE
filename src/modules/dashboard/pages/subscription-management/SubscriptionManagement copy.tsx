...
const SubscriptionManagement = () => {
  ...
  const savePlan = async () => {
    ...
    if (isCreating) {
      try {
        const features: SubscriptionFeature[] = cleanFeatures.map((feature, index) => ({
          id: Date.now() + index,
        ...
        const plan: createSubscriptionRequestDTO = {
          name: formData.name,
          description: formData.description,
          price: formData.price,
          discountPrice: formData.discountPrice || null,
        ...
        const createdPlan = await createSubscriptionAPI(plan)
        newPlan.id = createdPlan.id
  ...
  const getColorClass = (color: string, type: 'gradient' | 'bg' | 'text' | 'border' = 'gradient'): string => {
    const colorOption = colorOptions.find(opt => opt.value === color);
    if (type === 'gradient') return colorOption?.class || 'from-blue-600 to-blue-700';
    if (type === 'bg') return `bg-${color}-500/20`;
    if (type === 'text') return `text-${color}-300`;
    if (type === 'border') return `border-${color}-500/30`;
    return '';
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="max-w-7xl mx-auto p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent pb-4">
